...
class SpotifyRequestor {
  constructor(spotifyApi, timeRange, reportProgress) {
    this.s = spotifyApi;
  ...
  _runWithRetry(fn, actionDescription, retryCount = this.retryCount) {
    ...
    function tryRunPromise() {
      return fn().then(
        ...
        err => {
          console.log(
            `Error encountered. Current retryCount is = ${retryCount}`
          );
          if (retryCount > 0) {
            ...
            console.log(
              `Out of retries, failing the call: ${actionDescription}`
            );
  ...
  _makeRequestAndProcessRows(description, fn, rowProcessor, rowAccessor) {
    console.log(`Making request for ${description}`);
    rowAccessor = rowAccessor || (data => data.items);

    // Run this request using the retry logic we have
    return this._runWithRetry(fn, description).then(data => {
      console.log(
        `Received Results for ${description}. Number of rows: ${
          rowAccessor(data).length
        }`
      );
      const toRet = rowAccessor(data).map(rowProcessor);
      ...
      return Promise.resolve({ rows: toRet, paging });
    });
  }
  ...
  _makeRequestAndProcessRowsWithPaging(
    description,
    fn,
    rowProcessor,
    rowAccessor
    ...
    const getPage = (limit, offset) => {
      console.log(
        `Getting a page of data with limit=${limit} and offset=${offset}`
      );
      return this._makeRequestAndProcessRows(
        description,
        fn.bind(this, { limit, offset }), // bind the limit and offset in here
        rowProcessor,
        rowAccessor
      ).then(result => {
        const nextOffset = result.paging.offset + this.defaultPageSize;
        allRows = allRows.concat(result.rows);
        const totalRows =
          result.paging.total < this.maxResults
            ? result.paging.total
            : this.maxResults;

        console.log(
          `Received a page of data for ${description}. nextOffset is ${nextOffset}. totalRows is ${
            result.paging.total
          }. maxResults is ${this.maxResults}`
        );

        // Report our progress to the progress reporting function which was passed in
        this.reportProgress(
          `Received data for ${description}. Retrieved ${
        ...
        if (nextOffset < result.paging.total && nextOffset < this.maxResults) {
          ...
          console.log(
            `Done paging through results. Number of results was ${
              allRows.length
            }`
          );
  ...
  _getCollectionFromIds(
    ...
    console.log(
      `Retrieving a collection for ${description}. ${
        ids.length
      } ids are requested with blockSize ${blockSize}`
    );
    ...
    return Promise.all(promises).then(() => {
      console.log(
        `All requests have finished. Combining arrays together for ${description}`
      );
      const merged = [].concat(...resultBlocks);
      return merged;
    });
  }
  ...
  getMyTopArtists() {
    ...
    return this._makeRequestAndProcessRows(
      "getMyTopArtists",
      this.s.getMyTopArtists.bind(this, {
  ...
  getMyTopTracks() {
    ...
    return this._makeRequestAndProcessRows(
      "getMyTopTracks",
      this.s.getMyTopTracks.bind(this, {
        time_range: this.timeRange,
        limit: 50
      }),
      track => {
    ...
    return this._makeRequestAndProcessRowsWithPaging(
      ...
      albumObject => {
        ...
        return {
          ...
          image_link: albumObject.album.images[0]
            ? albumObject.album.images[0].url
            : null,
      ...
      savedAlbums // To get the rest of the albums, we first get all the user's saved tracks, then retrieve album
    ) =>
      // info for those tracks.
      this.getMySavedTracks().then(rows => {
        const allAlbums = [];
        const albums = rows.map(row => row.album_id);
        for (const i in albums) {
          if (!allAlbums.includes(albums[i])) {
            allAlbums.push(albums[i]);
          }
        }

        return this.getAlbums(allAlbums).then(addedAlbums => {
          const finalResults = savedAlbums.concat(addedAlbums);
    ...
    return this._makeRequestAndProcessRowsWithPaging(
      ...
      return this.getTrackFeatures(ids).then(trackFeatures => {
        ...
        for (let i = 0; i < trackFeatures.length; i++) {
          for (const attrname in trackFeatures[i]) {
  ...
  getMySavedArtists() {
    ...
    const allArtists = [];
    const appendArtists = rows => {
      const artists = rows.map(row => row.artist_id);
      for (const i in artists) {
        if (!allArtists.includes(artists[i])) {
    ...
    return Promise.all([
      ...
      spotifyRequestor.getMySavedTracks().then(appendArtists)
    ]).then(() => {
      console.log(
        `Finished finding artists in albums and tracks. Number of artists=${
          allArtists.length
        }`
      );
  ...
  getAlbums(ids) {
    ...
    return this._getCollectionFromIds(
      ...
      (
        album // Make sure this list is kept in sync with the saved albums endpoint
      ) => ({
